 ...
 *
 * grade-level-coaching 応答レベル保存失敗時の継続動作 統合テスト（タスク 8.2）
 *
 * 要件4.2「応答レベルの保存処理が失敗した場合（端末側のストレージが利用できない場合など）、
 * システムはチャット機能の利用をブロックせず、デフォルトまたは直前まで有効だった応答レベルで
 * 動作を継続する」を、ユニット（use-response-level.ts単体）ではなく
 * SettingsPage → ChatPage という実際の画面遷移フローを通しで検証する。
 *
 * localStorage.setItem をスローするようにモックし（Safari プライベートモード等の
 * QuotaExceededError を模擬）:
 *   1. SettingsPage で応答レベルを変更して保存しても、操作がクラッシュせず
 *      in-memory の表示が新しい選択値に更新されること（保存失敗はサイレントキャッチされる）
 *   2. その後 ChatPage でメッセージを送信しても、チャット機能自体はブロックされず
 *      （デフォルトの応答レベルにフォールバックして）正常に応答が返ってくること
 *
 * モックするのは真の外部境界のみ（grade-level-storage-failure.test.tsx と同じ方針）:
 ...
 *
 * use-response-level.ts / settings/page.tsx / chat/page.tsx は変更しない（実装済みコードをそのまま検証する）。
 *
 * カバー要件: 4.2
 */
...
describe("応答レベル保存失敗時の継続動作（タスク8.2、要件4.2）", () => {
  test("ストレージ書き込みが失敗しても設定画面の保存操作はクラッシュせず、UIには選択した値が即座に反映される", () => {
    render(<SettingsPage />);

    // 未設定時はデフォルト（基本）表示から開始する
    expect(screen.getByTestId("current-response-level").textContent).toBe("基本");
    ...
    expect(screen.getByTestId("current-response-level").textContent).toBe("応用");
  });

  test("設定保存が失敗した後でもチャット送信はブロックされず、デフォルトの応答レベルで応答が継続する", async () => {
    const mockCreate = setupGenAIMock();

    // 1. 設定画面で応答レベル変更を試みるが、ストレージ書き込みは失敗する
    ...
    // 永続化に失敗していたため、デフォルトの応答レベル（直前まで有効だった値）で
    // 動作が継続していることを確認する（要件4.2）
